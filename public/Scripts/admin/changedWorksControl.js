angular.module('changedWorksListPro',['changedWorksList','commonUI']);
angular.module('changedWorksList',[])
	
	.factory('worksInfo',['$http',function($http){
		return {
			//未处理数据
			loadData:function(cate,page,keys){
				if(cate == 0 || cate == undefined){
					return $http.get('/AdminWorks?haveEdit=true&haveChecked=0&pageWidth=5&pageIndex='+page);
				}else{
					return $http.get('/AdminWorks?haveEdit=true&haveChecked=0&cate='+cate+'&pageWidth=5&pageIndex='+page);
				}
				
			},
			//已处理数据
			loadDisposed:function(cate,page,recommend,keys){
				if(cate == 0 || cate == undefined){
					return $http.get('/AdminWorks?haveEdit=true&haveChecked=1&pageWidth=5&pageIndex='+page+(recommend==0?'':'&recommend='+recommend));
				}else{
					return $http.get('/AdminWorks?haveEdit=true&haveChecked=1&cate='+cate+'&pageWidth=5&pageIndex='+page+(recommend==0?'':'&recommend='+recommend));
				}
			},
			
			getClass:function(){
				return $http.get('/ClientData/WorksClass');
			},
			changeClass:function(worksid,userid,cate){
				return $http.get('/AdminWorks/ChangeCategory?worksID='+worksid+'&userID='+userid+'&cate='+cate);
			},
			//删除单个
			remove:function(worksid,userid){
				return $http.get('/AdminWorks/Remove?worksID='+worksid+'&userID='+userid);
			},
			//审核接口 status /0通过不推荐 2推荐至首页 4推荐
			recommend:function(worksid,userid,status){
				return $http.get('/AdminWorks/Recommend?worksID='+worksid+'&userID='+userid+'&status='+status);
			},
			//未通过的审核接口
			refuseCheck:function(worksid){
				return $http.get('/AdminWorks/RefuseCheck?worksID='+worksid);
			}
		}
	}])
	//自定义过滤器 jsonDate
	...
	.controller('worksListCtrl',
		['$scope','$http','worksInfo','$timeout',
		function($scope,$http,worksInfo,$timeout){
			...
			function processData(haveChecked,cate){
				...
				if(haveChecked==0){
					//加载未处理的数据
					worksInfo.loadData(cate,$scope.page,$scope.keywords).then(function(res){
						$scope.data = res.data;
					...
					//加载已处理的数据
					worksInfo.loadDisposed(cate,$scope.page,$scope.recommends).then(function(res){
						$scope.data = res.data;
			...
			function checkWhichThenProcess(){
				...
				//获取已选择的顶部分类选项
				var cate;
				$('.select_bar button').each(function(index,e){
					if($(e).attr('class') == 'btn btn-default sb_selected'){
						cate = index;
					}
				})
			...
			$scope.pageBtn = function(num){
				$scope.page = num;
				if($scope.inpModel && ($scope.keywords != $scope.inpModel)){
					$scope.keywords = $scope.inpModel;
					// processData(num,$scope.inpModel);
